                ...
                <div class="col-12">
                    <div class="card recent-sales overflow-auto">
                        <div class="card-body">
                            ...
                            <a href="{{route('fasilitas.create')}}" class="btn btn-outline-success btn-sm mb-2">Tambah Fasilitas</a>
                                    ...
                                    @foreach ($fasilitas as $key => $item)
                                        <tr>
                                            ...
                                            <td><span class="badge bg-success text-white text-wrap"
                                                    style="width: 7rem;">{{ $item->status }}</span></td>
                                            <td>
                                                @php
                                                    $id_fasilitas = App\Helper\HashHelper::encryptData($item->id);
                                                @endphp
                                                <a href="{{route('fasilitas.edit',$id_fasilitas)}}" class="btn btn-outline-primary btn-sm">Edit</a>
                                                <button type="button" class="btn btn-outline-danger btn-sm"
                                                    onclick="tampil('{{ route('fasilitas.destroy', $id_fasilitas) }}')">Delete</button>
                                            </td>
                                        </tr>
                                    @endforeach
